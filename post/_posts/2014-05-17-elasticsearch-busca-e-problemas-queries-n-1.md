...
Geralmente quando temos alguma aplicação que precisa de busca, ela contem relacionamentos entre tabelas, como por exemplo uma App de Loja, onde você tem produto relacionado com categoria, fabricante, etc. Se usarmos o Elasticsearch na aplicação, é possível que o banco de dados sofra com **queries N + 1**. É possível acabar com isso definindo alguns indices.
...
Então na view, chamamos os autores:
...
O que podemos fazer é adicionar um block com índices de autores:
...
Ainda, será necessário sobrescrever o método `to_indexed_json` para que possamos incluir autores:
...
Também, não precisamos mais fazer o elasticsearch carregar a partir do banco (puxar informações como objetos), então podemos tirar o trecho `(load: true)` do método `self.search` no model.
...
Sem informações vindas do banco. Muito mais rápido!
